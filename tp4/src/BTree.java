...
/**
 * B-tree est une structure de données arborescente qui
 * conserve les données triées et permet des recherches,
 * des accès séquentiels, des insertions et des suppressions en temps logarithmique.
 * L'arbre B est une généralisation d'un arbre de recherche binaire en ce sens qu'un
 * nœud peut avoir plus de deux enfants. Contrairement aux arbres de recherche binaires
 * auto-équilibrés, l'arbre B est optimisé pour les systèmes qui lisent et écrivent
 * de grands blocs de données. Il est couramment utilisé dans les bases de données et
